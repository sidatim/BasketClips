...
class NBAResponse:
    def __init__(self, response, status_code, url):
        ...
        print(f"Response status code from API:  " f"{status_code}" )
...
class NBAHTTP:
    ...
    def send_api_request(
        ...
        if proxy is None:
            request_proxy = st.session_state.get("proxy", None)
            print(f"{request_proxy} that will be used in NBA_API requests")
        elif not proxy:
        ...
        if isinstance(request_proxy, list):
            ...
            print("Selected proxy:", request_proxy)
            if DEBUG:
                print(request_proxy)
